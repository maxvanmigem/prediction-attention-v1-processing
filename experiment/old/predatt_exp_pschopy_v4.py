...
# #initialize window#
win = visual.Window(fullscr=True, color = 'black', colorSpace = 'rgb', units = 'pix', monitor= 'dell_system_monitor',useFBO=False)
...
stim_jitter_path = os.getcwd()+'/experiment_predatt_c1/stim_jitter.npy'
...
def fieldLocalizer(field,line_stim, stim_dur, isi_dur, mode = 'field', lab=lab):
    """
...
def generateQuadLocalizerTrials(n_stims, asynchronous=False, isi=.500 , jitter=.050):
    """
    ...
    localizer_prelist = np.repeat([0,1,2,3], n_stims/2, axis=0)
    random.shuffle(localizer_prelist)
    localizer_list = localizer_prelist[:n_stims]
    # Timing lists
    isi_list = np.repeat([isi],n_stims)
    jitter_list = np.random.default_rng().uniform(-jitter,jitter,n_stims)
    if asynchronous:
        for ind,t in enumerate(jitter_list):
            isi_list[ind] = (isi_list[ind] + t)/100
...
el_tracker.sendCommand("calibration_type = HV13") #13 point calibration (recommended for head free remote mode)
...
def terminate_task():
    ...
    if el_tracker.isConnected():
        ...
        try:
            el_tracker.receiveDataFile(edf_file, os.getcwd() + '/data/')
        except RuntimeError as error:
            print('ERROR:', error)
...
def checkGazeOnFix():
    ...
    while not trigger_fired:
        ...
        if core.getTime() - trigger_start_time >= 30.0:
...
stimset = generateStim(linelength=30,linewidth=2, coord_array=grid, colour='white',catch_colour='red',
...
field_stim = generateStim(linelength=30,linewidth=2, coord_array=field_grid, colour='white',catch_colour='red',
                               size=276, fixdistance=134, localizer= True)
## Trials
field_loc_trials, field_timing = generateFieldLocalizerTrials(3,3)
quad_loc_trials, quad_timing = generateQuadLocalizerTrials(8)
...
# Second localizer
cross.autoDraw = True
for ind,x in enumerate(quad_loc_trials):
    event.clearEvents(eventType = 'keyboard')
    fieldLocalizer(x,stimset,.1,.5)
    # Exit
    keys = event.getKeys()
    if 'escape' in keys:
        win.close()
