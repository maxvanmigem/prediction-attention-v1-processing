...
#options
n_trials = 8 # per block
n_predictible = 4
n_catch = 2
n_blocks = 2 # Has to be divisible by 4        
n_fieldloc = 1
n_quadreps = 1
# random generator
...
def stimPresentation(stimulus, stim_dur, isi_dur,iti_dur, start_quad, direction, q_catch, lab=lab): 
    """
...
def generateTrialList(tr_block, npredictable):
    ...
    # Make a matching array with regular (predicted) trials
    prediction_arr = startquad.copy()   # The 'last' argument for the end position in stimPresentation() is always relative to the first
    prediction_arr[:] = 0

    # Then make a part of the trials unpredictible  meaning that they apear in anticlockwise direction
    nunpred = tr_block - npredictable   
    prediction_arr[:nunpred] = 1 
...
def generateBlockStarts(n_blocks, ):
    """
    Generate a list with the start position for each block only for blocked design
    """
    if n_blocks % 2 == 0:   
        startquad = np.repeat(np.arange(2),repeats= int(n_blocks/2))
        random.shuffle(startquad)  # randomize
        return startquad
    else:
        print('n_blocks has to be divisible by 2')
        core.quit()
...
trial_list = []
tr_direction = np.empty(n_blocks,dtype=object)
catch_trials = np.empty(n_blocks,dtype=object)
for block in range(n_blocks):
    unsused_list, tr_direction[block] = generateTrialList(tr_block=n_trials,npredictable=n_predictible) #the unsused list was formarly known as start_pos
...
for i in range(n_blocks):
    ...
    for ind in range(n_trials):
        ...
        trials.addData('trial_direction',tr_direction[i][ind])
