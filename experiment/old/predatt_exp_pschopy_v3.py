...
for j in range(3):
    ...
    for i in range(3):
        grid = generateStimCoordinates(gridcol=12, gridrow=pilot_freq[j][i], jitter=linejitter_arr)
        stimset = generateStim(linelength=0.5, coord_array=grid, colour='white',catch_colour='red',size=pilot_size[j][i], fixdistance=pilot_pos[j][i] )
        ...
        for ind in range(n_trials):
            ...
            stimulus_times,trial_stamp = stimPresentation(stimulus=stimset, stim_dur=0.25 , isi_dur=0.5, iti_dur=0.501,
                                                        start_quad=start_pos[j+i][ind], 
                                                        last=last_pos[j+i][ind], 
                                                        q_catch=int(catch_trials[j+i][ind]))
            ...
            trials.addData('start_position',start_pos[j+i][ind])
            trials.addData('last_position',last_pos[j+i][ind])
            trials.addData('catch_trial',catch_trials[j+i][ind])
