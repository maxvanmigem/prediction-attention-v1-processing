...
# #initialize window#
win = visual.Window(fullscr=True, color = 'black', colorSpace = 'rgb', units = 'deg', monitor= 'dell_system_monitor',useFBO=False)
...
def generateStim(linelength, coord_array, colour, catch_colour, size, fixdistance):
    ...
    line_stimuli = np.empty((5,4), dtype=object)   # 5 types (0 is normal trials, 1-4 is catch stim on each quad respectively) and 4 quadrants
    ...
    for quad in range(4): 
        for row in range(gridrow):
            for col in range(gridcol):
                ...
                # Also one with catch stimuli
                catch_line_stim = visual.Line(win, start=(start_x, start_y), end=(end_x, end_y), lineColor=catch_colour)
                line_stimuli[quad+1,quad].append(catch_line_stim) # this only creates catch stimuli the rest of the catch arrays still need to be filled

        # Fill the empty cells of the catch arrays with normal stimuli 
        for i in np.arange(1,4):
            line_stimuli[quad+1,(quad+i)%4 ] = line_stimuli[0,(quad+i)%4]      
...
def stimPresentation(stimulus, stim_dur, isi_dur,iti_dur, start_quad, last, lab = lab): 
    """
...
if lab == 'actichamp':
    ...
    parallel.setPortAddress('0x3FB8')
...
if eye_tracking:
    ...
    FileNameET = 'pilot_predatt_' + info['Participant ID (***)'] + '.EDF' #open datafile (max 8 characters)
...
trial_list_pos = []
pos_start, pos_last = generateTrialList(10,5)
catch_trials = generateCatchTrials(10,5)
...
# trial_list_freq = []
# freq_start, freq_last = generateTrialList(2,1)
...
# trial_list_size = []
# size_start, size_last = generateTrialList(2,1)
...
for i in range(3):
    ...
    stimset = generateStim(linelength=0.5, coord_array=grid, colour='white',catch_colour='red',size=8, fixdistance=pilot_set[i] )
    ...
    for ind,trial in enumerate(pos_start):
        ...
        if eye_tracking:
            ...
            eyeTracker.sendCommand("record_status_message 'stim %s trial %s'" % (pos_start[ind], (trial_count))) #to show the current task, block nr and trial nr #+1 because Python starts at 0
        

        event.clearEvents(eventType = 'keyboard')
        my_clock.reset() #start rt timing
        stimulus_times,trial_stamp = stimPresentation(stimulus=stimset[catch_trials[ind],:], stim_dur=0.054 , isi_dur=0.5, iti_dur=0.501,
                                                      start_quad=pos_start[ind], last=pos_last[ind])
        keys = event.getKeys(timeStamped=my_clock)
        print(my_clock.getTime()*1000)
        #store data
        ...
        pos_trials.addData('trial', (trial_count)) #Python starts indexing at 
        ...
        if keys:
            ...
            pos_trials.addData('press_time',keys[-1][1]*1000)
        else:
...
#         my_clock.reset() #start rt timing
#         stimulus_time = stimPresentation(fix_cr=cross, stimulus=stimset, stim_dur=0.20 , isi_dur=0.5, 
...
#         my_clock.reset() #start rt timing
#         stimPresentation(fix_cr=cross, stimulus=stimset, stim_dur=0.20 , isi_dur=0.5, 
