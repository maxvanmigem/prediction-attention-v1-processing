...
# destination_path = "C:/Users/mvmigem/Documents/data/project_1/overlap_corrected/fixed-electrode/"
...
for (evp,rawp) in zip(event_dir_list,raw_dir_list)
    ...
    # selected electrode
    # selected_electrode = "POz"
    selected_electrode = string(evts[1,:selected_electrode])
    select!(data,selected_electrode)
    ...
    f = @formula 0 ~ 1 + position + sequence + expectation*attention 
    ...
    eff.selected_electrode .= selected_electrode
    # Save evoked
    subject_num = lpad(evts[1,:subject],2,"0")
    filename = "corrected_evoked_$subject_num.csv"
