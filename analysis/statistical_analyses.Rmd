...
# f1 <- P1 ~ 1 + expectation + attention + hemisphere +
...
# acc
```{r}
fit.behaviour_acc <-  lmer(dprime ~ 1 + attention + expectation + attention*expectation +
                           (1 | participant),
  data = df, 
  control = lmerControl(optimizer = c("Nelder_Mead"), optCtrl=list(maxfun=2e6)))
...
f1 <- dprime ~ 1 + expectation  +
                  (1 | participant)
f2 <- dprime ~ 1 + expectation  +
                  (1 | participant)

fit.behaviour_acc <- lmer(f1,data = df_attended, control = lmerControl(optimizer ='optimx', optCtrl=list(method='L-BFGS-B')))
confint(fit.behaviour_acc, method ="Wald")
summary(fit.behaviour_acc)
contrasts(df$attention)
contrasts(df$expectation)
