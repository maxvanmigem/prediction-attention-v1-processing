...
fit.c1_gen <- lmer(ultra_amp ~ 1 + expectation + attention + visual_field + 
...
```
```{r}
fit.c1_up <- lmer(ultra_amp ~ 1 + expectation + attention + attention*expectation +
...
```
```{r}
fit.c1_down <- lmer(ultra_amp ~ 1 + expectation + attention + attention*expectation +
                           (1| subject) + (attention | subject)+ (expectation| subject) ,
  data = df_down, 
  control = lmerControl(
  optimizer ='optimx', optCtrl=list(method='L-BFGS-B')))
