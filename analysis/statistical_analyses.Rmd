...
```
```{r}
fit.c1_tail <- lmer(tailored_up_amp ~ 1 + expectation + attention + attention*expectation +
                           (1| subject) + (attention | subject)+ (expectation| subject) ,
  data = df, 
  control = lmerControl(optimizer = c("bobyqa"), optCtrl=list(maxfun=2e9)))
...
```
```{r}
fit.c1_ultra <- lmer(ultra_down_amp ~ 1 + expectation + attention + attention*expectation +
                           (1| subject) + (attention | subject)+ (expectation| subject) ,
  data = df, 
  control = lmerControl(optimizer = c("Nelder_Mead"), optCtrl=list(maxfun=2e6)))
