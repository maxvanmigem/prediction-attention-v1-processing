...
df <- read.csv('C:/Users/mvmigem/Documents/data/project_1/compiled_dataframes/c1_long_20ms_df.csv')
...
fit.c1_gen <- lmer(tailored_amp ~ 1 + expectation + attention + visual_field + 
...
```
```{r}
fit.c1_up <- lmer(tailored_amp ~ 1 + expectation + attention + attention*expectation +
...
```
```{r}
fit.c1_down <- lmer(tailored_amp ~ 1 + expectation + attention,
  data = df_down, 
  control = lmerControl(
  optimizer ='optimx', optCtrl=list(method='L-BFGS-B')))
summary(fit.c1_down)
...
## full model
```{r}
fit.p3.full <-  lmer(mean_amp ~ 1 + attention + expectation + attention*expectation +
                           (1 | subject)+ (attention | subject)+ (expectation| subject),
  data = df, 
