...
f <- yhat ~ 1 + expectation + attention  +
                        attention*expectation +
                  (1 + attention + expectation  | subject)


fit.c1_1 <- lmer(f,data = df_ipsi, control = lmerControl(optimizer ='optimx', optCtrl=list(method='L-BFGS-B')))
fit.c1_2 <- lmer(f,data = df_contra, control = lmerControl(optimizer ='optimx', optCtrl=list(method='L-BFGS-B')))
# fit.c1_gen <- brm(f,data = df, family = gaussian())
check_model(fit.c1_1)
check_model(fit.c1_2)
anova(fit.c1_1,fit.c1_2)
