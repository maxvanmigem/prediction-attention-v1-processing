...
# First localizer
cross.autoDraw = True
eegTriggerSend(93,lab=lab) # signal start localizer
for ind,x in enumerate(field_loc_trials):
    ...
    if 'escape' in keys:
        ...
        win.close()
        core.quit()
eegTriggerSend(94,lab=lab) #signal end
# Pause recordings
if eye_tracking:
...
eegTriggerSend(95,lab=lab) # signal start
...
for i in quad_loc_set:
    ...
    if start_key == 'escape':
        ...
        win.close()
        core.quit()
    eegTriggerSend(96,lab=lab) # signal end localizer block
...
# Start loop for every quad
for i in quad_loc_set:
    eegTriggerSend(97,lab=lab) # signal start
    cross.autoDraw = True
    for j in range(n_quadreps):
        event.clearEvents(eventType = 'keyboard')
    ...
    if start_key == 'escape':
        ...
        win.close()
        core.quit()
    eegTriggerSend(98,lab=lab) # signal end localizer block
